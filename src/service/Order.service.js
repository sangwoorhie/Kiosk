...
import { OrderStatus } from '../db/models/OrderItems.js'


// message 파일
const unidentifieditem = new Message('상품 ID');
const unidentifiedorder = new Message('발주 ID')
const undefinedQuantity = new Message('수량');
...
class OrderService {
    ...
    editOrders = async (itemId, orderId, state) => {
        ...
        } else if (!Object.values(OrderStatus).includes(state)){
            return {
                status: 400,
        ...
        // 초기 발주상태 previous (발주 생성된것에서 찾기) (수정할 발주상태는 req.body에 입력한 state)
        const previous = await this.orderRepository.checkstatus(itemId, orderId);
        if(!previous){
        ...
        // 2, 3. Ordered (0) or Pending (1) → Canceled (3) : 조건없이 가능
        if(
            (previous.state == 0 && state == 'PENDING') || //Ordered (0) → Pending (1)
            (previous.state == 0 && state == 'CANCELED') || // Ordered (0) → Canceled (3)
            (previous.state == 1 && state == 'CANCELED') //  Pending (1) → Canceled (3)
        ){ 
        // OrderStatus는 결국 OrdserItem테이블의 state컬럼에 있는 배열임
        // 발주된 아이템의 상태 찾기
        const current = await this.orderRepository.updateorder(orderId, OrderStatus[state]) 
        return editstatus.status200();
        }
        
        // 초기 발주상테 → 수정된 발주상태
        // 4. Pending(1) → Completed(2) : 트랜젝션 적용, 완료와 동시에 상품 amount 증가
        if(previous.state == 1 && state == 'COMPLETED'){

            // 아이템 찾기
            const item = await this.orderRepository.checkitems(itemId);
            
            // 수량 업데이트 = 수정할 발주아이템의 수량 + 초기 발주아이템의 수량
            const updatedamount = item.amount + previous.amount
        ...
        // 7. Completed (2) → Ordered (0)
            ...
            (previous.state == 2 && state == 'CANCELED') ||
            (previous.state == 2 && state == 'PENDING') ||
            (previous.state == 2 && state == 'ORDERED')
        ){
            // 아이템 찾기
            const item = await this.orderRepository.checkitems(itemId);
